---
title: "Final exam - Recommendation System"
...
Moreover, we can see there is a ```{r eval=FALSE} genres``` effect on the ratings with the plot below :
...
We are still exploring, so we will keep the ```{r eval=FALSE} top_matrix``` in the following analysis.
...
We will finish this data exploration with a PCA. We will use the ```{r eval=FALSE}FactoMineR``` library. This time, we will try with 4 dimensions.
...
First, we randomly order the dataset. This ensures that the training set and test set are both random samples and that any biases in the ordering of the dataset are not retained in the samples. If we look carefully at the ```{r eval=FALSE}edx``` dataset, it is ordered by ```{r eval=FALSE}userId```. So we will remove this bias.
...
Our RMSE is 0.833 in just a few seconds. It is a pretty good result in a very short amount of time. We can calculate the processing time with the ```{r eval=FALSE}microbenchmark``` library and the following piece of code.
...
What about the L1 P and Q costs ? The default values in the tune option were ```{r eval=FALSE} c(0, 0.1)```. The best value selected for both costs was 0, but maybe 0.1 was a bit high as a regularization cost. Let's try with ```{r eval=FALSE} c(0, 0.01)```.
...
It seems that 0 is still the best value for ```{r eval=FALSE} costp_l1```. We get the same results for ```{r eval=FALSE} costq_l1```, so we won't show the code but the idea is the same. We keep 0 for both L1 costs.
...
During Data Exploration, we found latent factors, and thus the number of $k$ for matrix factorization during tuning. Of course, there are still improvements to do. We did not construct a linear model with all the different variables, like ```{r eval=FALSE}genres``` or ```{r eval=FALSE}timestamp```. There are also other calculation methods like AdaGrad, which adapts the learning rate according to the sparsity of each parameter. It increases for sparser parameters, and decreases with less sparse ones.

During my research, I was very interested in parallel computing and packages like ```{r eval=FALSE}H2O``` and ```{r eval=FALSE}SparklyR``` to increase computation speed. It is possible to train algorithms with ```{r eval=FALSE}H2O``` very quickly, whereas ```{r eval=FALSE}SparklyR``` was low and setting a good configuration was a bit hard. But it has a lot of algorithms, more than ```{r eval=FALSE}H2O```, and these packages can be really useful for large datasets like the Movielens dataset we used for this project.

The ```{r eval=FALSE}recommenderlab``` offers also a solution for matrix factorization and has many tools to build a recommendation system and to suggest movies to users.

We focused on one method in this paper that gives good results.

Less is more, but there is still a lot of work to do in order to build a better model and a lower RMSE.
